...
ua:template("etym-before",
    <template>
        ...
        <button onclick="{oxy:xquery-update('resources/xquery/firstEtymologicalNote.xql')}" style="visibility: {count(note) = 0};">Notă</button>
    </template>
),
...
ua:template("variantă-directă-latinizat.după-template",
    <template>
        <input data-ua-ref="{text()}" size="22" />
    </template>
),
ua:attach-template(ua-dt:css-selector("etym > idno[type = 'variantă-directă-latinizat.după'] ~ term"), "variantă-directă-latinizat.după-template"),
...
ua:template("etym-note-template",
    <template>
		Notă&amp;nbsp;
		<button onclick="{oxy:xquery-update('resources/xquery/etymologicalNote.xql')}" style="background-color: transparent;"><img src="../../resources/images/add.png" /></button>
        <button onclick="{oxy:xquery-update-action('deleteCurrentElement')}" style="background-color: transparent;" />
    </template>
),
...
ua:template("sense-def-before-template",
    <template>
        ...
        <button onclick="{oxy:xquery-update('resources/xquery/bibl-for-def.xql')}" data-showIcon="false" style="visibility: {let $context := . return count($context/following-sibling::bibl[@subtype = 'bibl-for-def' and @corresp = concat('#', $context/@xml:id)]) = 0};">Atestare definiție</button>        
